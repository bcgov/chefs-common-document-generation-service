...
| cronJob.activeDeadlineSeconds | int | `1800` | Wall-clock budget for the job, including pod scheduling and volume attach |
| cronJob.backoffLimit | int | `6` | Number of retries before the job is marked failed |
...
| cronJob.startingDeadlineSeconds | int | `300` | How long after the scheduled time a job may still be started |
